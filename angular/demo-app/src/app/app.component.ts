...
export class AppComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    (window as any).WriterControl_OnLoad = function (ctl: any) {
      ...
      //此方法要写在加载文档的前面
      (ctl as any).EventElementMouseClick = function (
        eventSender: any,
        args: any
      ) {
        args.Handled = true;
        console.log('元素点击事件', args);
      };
      (ctl as any).ondocumentclick = function (eventSender: any) {
        console.log('点击事件');
      };
    };
    //请求js并创建编辑器
    (window as any)._DCWriter5SpecifyBasePath =
      'http://www.dcwriter.cn:8023/ServicePage.aspx?wasmres=dcwriter5.js';
    (window as any)._DCWriter5SpecifyServicePageUrl =
      'http://www.dcwriter.cn:8023/ServicePage.aspx?wasmres=dcwriter5.js';
    const scriptUrls = [
      'http://www.dcwriter.cn:8023/ServicePage.aspx?wasmres=dcwriter5.js',
      'http://www.dcwriter.cn:8023/Jquery/jquery-1.7.2.min.js',
    ];
