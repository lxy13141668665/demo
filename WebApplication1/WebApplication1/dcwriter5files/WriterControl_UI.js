    ...
    */
    ...
    BeginEditValueUseListBoxControl: function (
        ...
        function InnerShowListBoxControl() {
            ...
            if (currentInput != null && (currentInput.InnerMultiSelect === true || currentInput.InnerMultiSelect == 'true')) {
                listBox = WriterControl_ListBoxControl.CreateMultiSelectControl(listItems, rootElement, args.OldText, args.OldValue, currentInput);
            } else {
                listBox = WriterControl_ListBoxControl.CreateListBoxControl(listItems, callBack, args.OldText, args.OldValue, currentInput);
            }
            if (currentInput != null && currentInput.DynamicListItems != null && (currentInput.DynamicListItems === true || currentInput.DynamicListItems == 'true') && args.NeedSearchInput === true) {
                if (listBox != null) {
                    //在此之前先插入一个搜索框
    ...
    ShowCalculateControl: function (containerID, intPageIndex, intLeft, intTop, intHeight, dblInputValue) {
        ...
        var callBack = function (newInputValue) {
            if (newInputValue != dblInputValue) {
                // 用户确认操作后执行函数
                var ctlRef = DCTools20221228.GetDCWriterReference(containerID);
                if (ctlRef != null) {
    ...
    GetClipboardData: async function (e, rootElement) {
        ...
        if (e != null && e.clipboardData != null) {
            //存在文件信息
            if (e.clipboardData.files.length > 0 && e.clipboardData.types.length == 1 && e.clipboardData.types[0].indexOf('image/') == 0) {
                Promise.all(function* () {
                    ...
                    results.forEach(data => {
                        dataList.push(data.type, data.result);
                    })
    ...
    ShowCaret: function (containerID, intPageIndex, intDX, intDY, intWidth, intHeight, bolVisible) {
        ...
        if (txtEdit == null) {
            txtEdit = document.createElement("TEXTAREA");
