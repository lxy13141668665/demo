        ...
        rootElement.SetAutoZoom = function (callback, eventArgs, noAddAttr) {
            ...
            if (hasAutoZoom != null && (hasAutoZoom == 'true' || hasAutoZoom == 'True')) {
                ...
                rootElement.rootResizeTime = setTimeout(function () {
                    ...
                    var hasCanvas = pageContainer.querySelector('canvas[dctype=page]');
                    if (hasCanvas) {
                        var pageWidth = pageContainer.clientWidth;
                        var canvasWidth = Number(hasCanvas.getAttribute('native-width')) + 12;
                        if (pageWidth - canvasWidth > 0) {
                            var zoomRate = rootElement.__DCWriterReference.invokeMethod("get_ZoomRate");
                            zoomRate = Number(pageWidth / canvasWidth).toFixed(7);
                            //获取到canvas的宽度,此处用这个是因为SetZoomRate中处理autozoom
                            WriterControl_UI.EditorSetZoomRate(rootElement, zoomRate);
                        }
                    }
                    clearTimeout(rootElement.rootResizeTime);
                    delete rootElement.rootResizeTime;
